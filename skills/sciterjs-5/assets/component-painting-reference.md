...
An **aspect** is a plain JS function attached to an element via the CSS `aspect:` property — a lighter-weight alternative to a full `prototype:` class component when you just need to bolt some behavior onto stock elements (e.g. every element with an `onclick` attribute, or a whole class of `<chart>` elements) without defining a class.
...
function Donut({fill, thickness}) {
  ...
  this.paintContent = function(gfx) {
    // ... draw donut chart using gfx, fill, thickness ...
  };
}
```

Contrast with `prototype:` (class) components: aspects are one-shot setup functions with no `componentDidMount`/`componentWillUnmount` lifecycle of their own — they piggyback on the element's own mount moment and typically just wire up `paintContent`/`paintBackground` or event listeners directly. Use `prototype:` when you need a reusable class with lifecycle hooks and JSX rendering; use `aspect:` for terse, functional, parametrized attachments driven straight from CSS.
