...
- Custom-styled built-in dialog: `styleset={__DIR__ + "custom.css#alert!"}` (the trailing `!` — from the sample — forces override of the built-in styleset rules).
...
Both use `this.box("border","parent")` (box relative to the *parent* element) while resizing, and call `Window.this.update()` after each `style.set()` to force a synchronous repaint rather than waiting for the next frame.
...
- **`behavior: native-textarea`** — wraps an actual native OS edit control (Win32 `EDIT` class) as a Sciter element. Gotcha called out directly in the sample: hide it with `visibility:hidden`, **not** `display:none` — being a real overlaid native child window, `display:none` styling doesn't reliably hide the underlying OS window the way it does for normal Sciter elements.
